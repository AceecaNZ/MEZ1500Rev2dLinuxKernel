...
// 20140329 SV: Hack - values from bootloader	
regs->lcdcon1   = 0x878;
regs->lcdcon2   = 0x84fc501;
regs->lcdcon3   = 0x28ef0f;
regs->lcdcon4   = 0x5;
regs->lcdcon5   = 0xf09;
...
// 2011-04-12 SV: Added
static int s3c2410fb_save(struct fb_info *info)
{
	unsigned map_size = PAGE_ALIGN(info->fix.smem_len);
	unsigned int x;
	char *p;

	buf = kmalloc(map_size, GFP_KERNEL);
	if (!buf)
	{
		printk(KERN_ERR "s3c2410fb_save() Failed to allocate mem\n");
		return -ENOMEM;
	}
...
// 2011-04-12 SV: Added
static int s3c2410fb_restore(struct fb_info *info)
{
	unsigned map_size = PAGE_ALIGN(info->fix.smem_len);
	unsigned int x;
	char *p;
	...
	if (!buf)
	{
		printk(KERN_ERR "s3c2410fb_restore() No buf\n");
		return -ENOMEM;
	}
